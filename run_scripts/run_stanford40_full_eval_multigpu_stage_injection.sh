...
BASE_OUTPUT_TAG="${BASE_OUTPUT_TAG:-stanford40_full_mgpu_stage_injection_$(date +%Y%m%d_%H%M%S)}"
RUN_ONLY="${RUN_ONLY:-}"
STAGES_TO_EXPORT="${STAGES_TO_EXPORT:-hpool_clean hpool_priv}"
...
for injection_stage in "${INJECTION_STAGES[@]}"; do
  ...
  env \
    ...
    EXTRACT_STAGES="$STAGES_TO_EXPORT" \
    EXTRA_PROBE_ARGS="$EXTRA_PROBE_ARGS" \
    EXTRA_EXTRACT_ARGS="--injection-stage $injection_stage $EXTRA_EXTRACT_ARGS" \
    bash "$ROOT_DIR/run_scripts/run_stanford40_full_eval_multigpu.sh" 2>&1 | tee -a "$log_path"
...
for path in sorted(summary_dir.glob("*.json")):
    ...
    for key in ["hpool_clean_global", "hpool_priv_global", "hpool_clean_nonhead", "hpool_priv_nonhead"]:
        metrics = payload.get("results", {}).get(key, {})
        record[f"{key}_test_acc"] = metrics.get("test", {}).get("accuracy")
        record[f"{key}_test_macro_f1"] = metrics.get("test", {}).get("macro_f1")
...
headers = [
    ...
    "clip_norm",
    "hpool_clean_global_test_acc",
    "hpool_clean_global_test_macro_f1",
    "hpool_priv_global_test_acc",
    "hpool_priv_global_test_macro_f1",
    "hpool_clean_nonhead_test_acc",
    "hpool_clean_nonhead_test_macro_f1",
    "hpool_priv_nonhead_test_acc",
    "hpool_priv_nonhead_test_macro_f1",
]
