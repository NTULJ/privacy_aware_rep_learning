...
POSS=("vit_hidden_24")  # 可选: vit_after_pos_embed vit_hidden_x vit_output
RUN_TS="$(date +%Y%m%d_%H%M%S)"
mkdir -p "${LOG_ROOT}/${RUN_TS}"

for pos in "${POSS[@]}"; do
    MODEL_DIR="${MODEL_ROOT}/eps${EPSILON}_pos-${pos}_norm1.0_delta1e-5_step915"
    LOG_FILE="${LOG_ROOT}/${RUN_TS}/eps${EPSILON}_${pos}.log"
    ...
    python "${PROJECT_ROOT}/scripts/evaluation/eval_qwen3_vl_models.py" \
        ...
        --save_details ./results/details.jsonl \
        --tasks falldown
        > "$LOG_FILE" 2>&1
done
