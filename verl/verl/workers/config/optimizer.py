...
def build_optimizer(parameters, config: FSDPOptimizerConfig):
    ...
    Args:
        parameters: Model parameters to optimize, or an iterable of param groups / dicts.
            If an element is a dict (param group), it is passed through as-is;
            otherwise parameters are treated as a single default group.
    ...
    try:
        mod = importlib.import_module(config.optimizer_impl)
        optimizer_cls = getattr(mod, config.optimizer)
        ...
        raise AttributeError(
            ...
            f"Available optimizers: {dir(mod)}"
        ) from e

    # Support pre-built param groups (list of dicts) for per-group config
    # such as different weight_decay for DP MLP params.
