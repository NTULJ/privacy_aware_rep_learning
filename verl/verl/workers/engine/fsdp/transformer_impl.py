...
class FSDPEngine(BaseEngine):
    ...
    def _build_optimizer(self, module):
        ...
        # Split parameters into two groups by name:
        # 1. DP MLP params (name starts with "_dp_mlp_") — weight_decay=0.0
        # 2. All other params — weight_decay from config
        # We use name matching instead of _dp_mlp_param attribute because
        # FSDP2 wrapping may replace parameter objects, losing custom attributes.
        mlp_params = []
        other_params = []
        for name, p in module.named_parameters():
            if name.startswith("_dp_mlp_") or getattr(p, '_dp_mlp_param', False):
                mlp_params.append(p)
            else:
        ...
        if mlp_params:
            param_groups = [
                {"params": other_params},
                {"params": mlp_params, "weight_decay": 0.0},
            ]
            if self.rank == 0:
                print(f"[FSDPEngine] Building optimizer with 2 param groups: "
                      f"{len(other_params)} regular params, {len(mlp_params)} DP MLP params (weight_decay=0.0)")
        else:
            param_groups = module.parameters()
            if self.rank == 0:
                # Debug: print all param names containing "dp_mlp" or "mlp"
                mlp_names = [n for n, _ in module.named_parameters() if 'dp_mlp' in n or 'mlp' in n.lower()]
                print(f"[FSDPEngine] No DP MLP params found. Names with 'dp_mlp/mlp': {mlp_names}")
