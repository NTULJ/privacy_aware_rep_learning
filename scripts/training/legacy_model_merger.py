...
class FSDPModelMerger(BaseModelMerger):
    ...
    def merge_and_save(self):
        ...
        mlp_keys = [k for k in merged_state_dict if k.startswith("_dp_mlp_")]
        if mlp_keys:
            mlp_state = {k: merged_state_dict.pop(k) for k in mlp_keys}
            os.makedirs(self.config.target_dir, exist_ok=True)
            mlp_path = os.path.join(self.config.target_dir, "dp_mlp_weights.pt")
            torch.save(mlp_state, mlp_path)
            print(f"Saved {len(mlp_keys)} DP MLP weight(s) to {mlp_path}")
    ...
    def _test_state_dict(self, state_dict: dict[str, torch.Tensor]):
        ...
        extra_keys = {k for k in extra_keys if not k.startswith("_dp_mlp_")}
