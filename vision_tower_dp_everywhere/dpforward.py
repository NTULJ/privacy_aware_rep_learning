...
def get_noise_multiplier(
    ...
    if local_dp:
        if noise_type == 'aGM':
            return matrix_gaussian_noise(epsilon=eps, delta=delta, sensitivity=sensitivity)
        elif noise_type == 'GM':
            return Gaussian_noise(epsilon=eps, delta=delta, sensitivity=sensitivity)
...
def add_noise(embeddings, noise_factor=0.5, norm_c=1.0, add_noise=True):
    if add_noise:
        ...
        # 【核心修复】：使用 torch.randn，并严格对齐原张量的 dtype 和 device
        noise_embeds = noise_factor * torch.randn(
            size=embeddings.shape, 
            dtype=embeddings.dtype,   # 强制设为 bfloat16 (跟随输入)
            device=embeddings.device  # 直接在目标显卡上生成，省去 .to() 的显存搬运开销
        )
...
def add_noise_with_fixed_pca(
    ...
    if x.ndim == 3:
        ...
        raise RuntimeError(f"Unsupported PCA noise shape: {list(orig_shape)}")

    mean = mean.to(device=x.device, dtype=torch.float32).reshape(1, -1)
    basis = basis.to(device=x.device, dtype=torch.float32)   # [r, d]

    if x2.shape[1] != basis.shape[1]:
        raise RuntimeError(
            f"PCA dim mismatch: input dim={x2.shape[1]}, basis dim={basis.shape[1]}"
        )
    ...
    # 2) project
    z = x_centered @ basis.t()   # [N, r]

    # 3) token-level clip in PCA subspace
    z_norm = torch.norm(z, dim=-1)
    clip_coef = norm_c / (z_norm + 1e-6)
    clip_coef = torch.clamp(clip_coef, max=1.0)
    z_clipped = z * clip_coef.unsqueeze(-1)

    # 4) add noise in PCA subspace
    noise = noise_factor * torch.randn_like(z_clipped)
    z_noisy = z_clipped + noise

    # 5) reconstruct
    x_recon = z_noisy @ basis + mean

    if orig_shape != x_recon.shape:
        x_recon = x_recon.reshape(orig_shape)

    return x_recon.to(orig_dtype)